...
pub fn app(state: &'static ai_routes::AppState) -> Router {
    Router::new()
        ...
        .route(
            "/api/sources/{id}/hide",
            axum::routing::post(sources::toggle_hidden),
        )
        // Knowledge Points
        .route(
            "/api/knowledge",
            get(knowledge::list_kps).post(knowledge::create_kp),
        )
        .route(
            "/api/knowledge/{id}",
            get(knowledge::get_kp).delete(knowledge::delete_kp),
        )
        .route(
        ...
        .route(
            "/api/relations/{id}",
            axum::routing::delete(relation::delete_relation),
        )
        // Quiz
        ...
        .route(
            "/api/learning/plans",
            get(learning::list_plans).post(learning::create_plan),
        )
        .route("/api/learning/reviews/due", get(learning::get_due_reviews))
        // AI
        .route(
            "/api/ai/research",
            axum::routing::post(ai_routes::start_research),
        )
        .route(
            "/api/ai/generate-quiz",
            axum::routing::post(ai_routes::generate_quiz),
        )
        .route(
            "/api/ai/update-mastery",
            axum::routing::post(ai_routes::update_mastery),
        )
        ...
        .route(
            ...
            axum::routing::delete(chat_routes::delete_session),
        )
        .route(
            "/api/chat/sessions/{id}/messages",
            get(chat_routes::get_messages),
        )
        .route(
        ...
        .route(
            "/api/settings/provider-kinds",
            get(settings::list_provider_kinds),
        )
        .route(
        ...
        .route(
            "/api/settings/providers/{provider_id}/models/{model_id}",
        ...
        .route(
            "/api/settings/tasks/{task_name}",
            axum::routing::put(settings::set_task_model),
        )
        .route(
            "/api/settings/general",
            axum::routing::put(settings::update_general),
        )
        .route(
            "/api/settings/test-connection",
            axum::routing::post(settings::test_connection),
        )
        // Logs
    ...
    CorsLayer::new()
        .allow_origin([
            "http://localhost:14200"
                ...
                .unwrap(),
            // Tauri webview origins. Windows serves the packaged app from
            // http://tauri.localhost (with an https variant), other platforms
            // from tauri://localhost. The desktop build reaches the backend by
            // absolute loopback URL, so all three must be allowed or the
            // browser engine blocks the response.
...
async fn audit_middleware(request: axum::http::Request<axum::body::Body>, next: Next) -> Response {
    ...
    if status_code >= 400 && path != "/api/logs/batch" {
        ...
        if crate::error::is_internal(&text) {
            ...
            response =
                Response::from_parts(parts, Body::from(crate::error::GENERIC_INTERNAL_MESSAGE));
        } else {
